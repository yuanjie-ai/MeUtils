...
if __name__ == '__main__':
    ...
    for file in os.listdir(dir):
        ...
        print(filename)
        # filename = './o32/投资管理系统O3.2_风控.pdm'
        ...
        for pkg in PDMHandler.getPkgNodes(ph):
            ...
            if not PDMHandler.getTblNodesInPkg(pkg):
                continue
            tbl_list = []
            for tbl in PDMHandler.getTblNodesInPkg(pkg):
                ...
                for col in PDMHandler.getColNodesInTbl(tbl):
                    ...
                    col_list.append(col_attrs)
                df = pd.DataFrame(col_list)
                df['tbl_Name'] = tbl_attrs["Name"]
                df['tbl_Code'] = tbl_attrs["Code"]
                df['tbl_Creator'] = tbl_attrs["Creator"]
                df['tbl_Comment'] = tbl_attrs["Comment"]
                tbl_list.append(df)
            df_tbl = pd.concat(tbl_list)
            df_tbl['pkg_Name'] = pkg_attrs["Name"]
            df_tbl['pkg_Code'] = pkg_attrs["Code"]
            df_tbl['pkg_Creator'] = pkg_attrs["Creator"]
            pkg_list.append(df_tbl)
        df_res = pd.concat(pkg_list)
        df_res = df_res[['pkg_Name', 'pkg_Code', 'pkg_Creator', 'tbl_Name', 'tbl_Code','tbl_Creator',
                         'tbl_Comment','Name', 'Code', 'CreationDate', 'Creator', 'ModificationDate',
                         'Modifier','DataType', 'Length', 'Column.Mandatory', 'Comment']]
        df_res.to_excel("./data/"+file.replace('pdm','xlsx'),index=False)
