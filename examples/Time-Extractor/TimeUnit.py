...
class TimeUnit:
    ...
    def norm_checkKeyword(self):
        ...
        if match is not None:
            if 0 <= self.tp.tunit[3] <= 11:
                self.tp.tunit[3] += 12
            if self.tp.tunit[3] == -1:  # 增加对没有明确时间点，添加“傍晚”这种情况的处理
                self.tp.tunit[3] = RangeTimeEnum.evening
            # 处理倾向于未来时间的情况
            self.preferFuture(3)
            self.isAllDayTime = False
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            if 5 <= self.tp.tunit[3] <= 11:  # new: 修改夜里三点处理为下午三点的情况
                self.tp.tunit[3] += 12
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            if 9 <= self.tp.tunit[3] <= 11:  # new: 修改夜里三点处理为下午三点的情况
                self.tp.tunit[3] += 12
            elif self.tp.tunit[3] == 12:
                self.tp.tunit[3] = 0
            elif self.tp.tunit[3] == -1:  # 增加对没有明确时间点，只写了“下午|午后”这种情况的处理
                self.tp.tunit[3] = RangeTimeEnum.midNight
            # 处理倾向于未来时间的情况
            self.preferFuture(3)
            self.isAllDayTime = False
    ...
    def norm_setSpecial(self):
        ...
        :return:
        """
        rule = u"(晚上|夜间|夜里|半夜|今晚|明晚|晚|夜里|下午|午后)(?<!(周|星期))([0-2]?[0-9]):[0-5]?[0-9]:[0-5]?[0-9]"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            ...
            rule = u"(晚上|夜间|夜里|半夜|今晚|明晚|晚|夜里|下午|午后)(?<!(周|星期))([0-2]?[0-9]):[0-5]?[0-9]"
            pattern = re.compile(rule)
            match = pattern.search(self.exp_time)
            if match is not None:
        ...
        if match is None:
            rule = u"(?<!(周|星期|晚上|夜间|半夜|夜里|今晚|明晚|晚|夜里|下午|午后))([0-2]?[0-9]):[0-5]?[0-9]:[0-5]?[0-9]"
            pattern = re.compile(rule)
            match = pattern.search(self.exp_time)
            if match is not None:
                tmp_target = match.group()
                ...
                rule = u"(?<!(周|星期|晚上|夜间|夜里|半夜|今晚|明晚|晚|夜里|下午|午后))([0-2]?[0-9]):[0-5]?[0-9]"
                pattern = re.compile(rule)
                match = pattern.search(self.exp_time)
                if match is not None:
                    tmp_target = match.group()
    ...
    def norm_setCurRelated(self):
        ...
        rule = u"(?<=(前))[0-9](?=(天))|([0-9]+)(?=((天)((前)|(之前)|(以前))))"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            flag[2] = True
            cur = cur.shift(days = -int(match.group()))
